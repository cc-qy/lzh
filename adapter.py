...
DATA_PATH = '/home/cc_user/waymo/training/'
# path to save kitti dataset
KITTI_PATH = '/home/cc_user/waymo/kitti_format/waymo/training/'
...
class Adapter:
    ...
    def cvt(self):
        ...
        #     print((self.__file_names[file_idx]))
        #     # print(file_name)
        # xxx
        for file_idx, file_name in enumerate(self.__file_names):
            print('File {}/{}'.format(file_idx, len(self.__file_names)))
            ...
            frame_num = 0
            self.create_folder_seq(file_name)
            for data in dataset:
                # print (len(dataset))
                ...
                # save the image:
                # s1 = time.time()
                self.save_image(frame, frame_num, file_name)
                # e1 = time.time()

                # parse the calib
                # s2 = time.time()
                self.save_calib(frame, frame_num, file_name)
                # e2 = time.time()

                # parse lidar
                # s3 = time.time()
                self.save_lidar(frame, frame_num, file_name)
                # e3 = time.time()

                # parse label
                # s4 = time.time()
                self.save_label(frame, frame_num, file_name)
                # e4 = time.time()
                # if file_idx==1:
                #     xxx
    ...
    def save_image(self, frame, frame_num, file_name):
        ...
        for img in frame.images:
            img_path = IMAGE_PATH + str(img.name - 1) + '/' +file_name.split('/')[-1]+'/' + str(frame_num).zfill(INDEX_LENGTH) + '.' + IMAGE_FORMAT
    ...
    def save_calib(self, frame, frame_num, file_name, kitti_format=True):
        """ parse and save the calibration data
                :param frame: open dataset frame proto
                :param frame_num: the current frame number
                :return:
        """
        fp_calib = open(CALIB_PATH + '/' +file_name.split('/')[-1]+'/'+ str(frame_num).zfill(INDEX_LENGTH) + '.txt', 'w+')
    ...
    def save_lidar(self, frame, frame_num, file_name):
        ...
        points, intensity, frame_pose = self.convert_range_image_to_point_cloud(
            frame,
            range_images,
        ...
        pc_path = LIDAR_PATH + '/' +file_name.split('/')[-1]+'/'+ str(frame_num).zfill(INDEX_LENGTH) + '.bin'
        point_cloud.tofile(pc_path)

        pose = open(POSE_PATH + '/' +file_name.split('/')[-1]+'/'+ str(frame_num).zfill(INDEX_LENGTH) + '.txt', 'a')
        pose.write(str(frame_pose))
        # pose.write(str(frame_pose[0][1]))
        # pose.write(str(frame_pose[0][2]))
        # pose.write(str(frame_pose[0][3]))
        pose.close()

        # calib_path = POSE_PATH + '/' + str(frame_num).zfill(INDEX_LENGTH) + '.txt'
        # f  = open(calib_path,'r')
        # lines = f.readlines()
        # print (str(lines[0][1:]).split()[0])
        # print (str(lines[0][1:]).split()[1])
        # print (str(lines[0][1:]).split()[2])
        # print (str(lines[0][1:]).split()[3])
        # print (lines[1])
        # print (lines[2])
        # print (lines[3][:-1])

    def save_label(self, frame, frame_num, file_name, kitti_format=True):
        """ parse and save the label data in .txt format
                :param frame: open dataset frame proto
                :param frame_num: the current frame number
                :return:
                """
        fp_label_all = open(LABEL_ALL_PATH + '/' +file_name.split('/')[-1]+'/'+ str(frame_num).zfill(INDEX_LENGTH) + '.txt', 'w+')
        ...
        for obj in frame.laser_labels:
            ...
            # store the label
            fp_label = open(LABEL_PATH + name + '/' +file_name.split('/')[-1]+'/'+ str(frame_num).zfill(INDEX_LENGTH) + '.txt', 'a')
            fp_label.write(line)
            fp_label.close()
    ...
    def create_folder_seq(self,file_name):
        # if not os.path.exists(KITTI_PATH):
        #     os.mkdir(KITTI_PATH)
        seq_num = file_name.split('/')[-1]
        if not os.path.exists(CALIB_PATH+'/'+seq_num):
            os.mkdir(CALIB_PATH+'/'+seq_num)
        if not os.path.exists(LIDAR_PATH+'/'+seq_num):
            os.mkdir(LIDAR_PATH+'/'+seq_num)
        if not os.path.exists(LABEL_ALL_PATH+'/'+seq_num):
            os.mkdir(LABEL_ALL_PATH+'/'+seq_num)
        if not os.path.exists(POSE_PATH+'/'+seq_num):
            os.mkdir(POSE_PATH+'/'+seq_num)
        if not os.path.exists(VIEW_PATH+'/'+seq_num):
            os.mkdir(VIEW_PATH+'/'+seq_num)
        for i in range(5):
            if not os.path.exists(IMAGE_PATH + str(i)+'/'+seq_num):
                os.mkdir(IMAGE_PATH + str(i)+'/'+seq_num)
            if not os.path.exists(LABEL_PATH + str(i)+'/'+seq_num):
                os.mkdir(LABEL_PATH + str(i)+'/'+seq_num)
    ...
    def convert_range_image_to_point_cloud(self, frame, range_images, range_image_top_pose, ri_index=0):
        ...
        # print ("frame_pos:", np.array(frame.pose.transform),np.array(frame.pose.transform).shape)
        # print ("frame_pose:", np.reshape(np.array(frame.pose.transform), [4, 4]))
        # [H, W, 6]
        ...
        return points, intensity, np.array(frame.pose.transform)
